...
const ServiceCard: React.FC<ServiceProps> = ({
  ...
  return (
    <div 
      className={`service-card animate-fade-in ${isPopular ? 'border-[#FFB5C2] border-2' : ''}`}
      style={{ transitionDelay: delay }}
    >
      {isPopular && (
        <div className="bg-[#FFF07C] text-white px-4 py-1 inline-block mb-4">
          Most Popular
        </div>
      )}
        ...
        <ul className="space-y-3 mb-8">
          {features.map((feature, index) => (
            <li key={index} className="flex items-start">
              <CheckCircle size={18} className="text-[#FFB5C2] mr-2 mt-1 flex-shrink-0" />
    ...
    <section id="services" className="section bg-white">
      <div className="container">
        <div className="max-w-3xl mx-auto text-center mb-16 animate-fade-in">
          <h2 className="text-3xl md:text-4xl font-semibold mb-6">My Services</h2>
          <p className="text-lg text-gray-700">
            I offer personalized coaching packages designed to meet you where you are and guide you to where you want to be.
          </p>
        </div>
